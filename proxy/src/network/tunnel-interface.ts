...
// 固定できず、かつ経路の切替方式もベンダー依存である。実機検証で、AdGuard VPN CLI（TUNモード）は
// メインテーブルのデフォルトルートを書き換えず、ポリシールーティング（`ip rule`で専用テーブルを
// 優先参照）で全通信をトンネルへ向けることが判明したため、`ip route show default`（メインテーブルのみ
// 参照）では接続を検知できない。`ip route get <公開IP>`はポリシールーティングを含めたカーネルの実際の
// 経路選択結果を返すため、どちらの方式のベンダーにも対応できる
