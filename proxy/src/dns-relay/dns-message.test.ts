...
import { buildServfail, parseAnswer, parseQuery, truncateForUdp, udpLimitFor } from "./dns-message.js";
...
describe("parseAnswer", () => {
  ...
  it("RCODEを返し、Aレコードが無ければ空", () => {
    ...
    expect(answer).toEqual({ rcode: 3, truncated: false, aRecords: [] });
  });
...
describe("buildServfail", () => {
  it("IDと質問を引き継いだSERVFAIL（RCODE=2）を返す", () => {
    ...
    expect(parseAnswer(response)).toEqual({ rcode: 2, truncated: false, aRecords: [] });
  });
});
