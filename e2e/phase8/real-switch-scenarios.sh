...
api() { curl -s -m 60 -b "$E2E_COOKIE_JAR" -X "$1" ${3:+-H 'content-type: application/json' -d "$3"} "$BASE_URL/api$2"; }
...
restore() {
  ...
  # 通常構成のapiが起動するのを少し待ってから、Web UI利用者アカウントを削除しインストール直後の未設定状態へ戻す。
  sleep 5
...
wait_for 60 'curl -sf -m 5 "$BASE_URL/api/v1/operator" >/dev/null 2>&1'
e2e_api_login "$BASE_URL"
...
check "切替のAPIが成功する（200）" '[ "$(curl -s -b "$E2E_COOKIE_JAR" -o /dev/null -w "%{http_code}" -m 90 -X PUT -H "content-type: application/json" -d "{\"providerId\":\"mockproton\"}" $BASE_URL/api/v1/providers/active)" = 200 ]'
...
check "切り戻しのAPIが成功する（200）" '[ "$(curl -s -b "$E2E_COOKIE_JAR" -o /dev/null -w "%{http_code}" -m 60 -X PUT -H "content-type: application/json" -d "{\"providerId\":\"adguardvpn\"}" $BASE_URL/api/v1/providers/active)" = 200 ]'
