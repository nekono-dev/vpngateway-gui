...
cleanup() { $DC down -v >/dev/null 2>&1; rm -rf "$E2E_VENDORS_DIR" "e2e/vendors/$NEW"; e2e_api_cleanup; }
trap cleanup EXIT
...
e2e_api_login "$BASE"
for _ in $(seq 1 60); do api GET /v1/providers >/dev/null 2>&1 && break; sleep 1; done
...
check "選択できる（PUT /v1/providers/active が200）" test "$(curl -s -b "$E2E_COOKIE_JAR" -o /dev/null -w '%{http_code}' -m 30 -X PUT -H 'content-type: application/json' -d "{\"providerId\":\"$NEW\"}" "$BASE/api/v1/providers/active")" = 200
...
for _ in $(seq 1 60); do curl -sf "$BASE/api/v1/operator" >/dev/null 2>&1 && break; sleep 1; done
# apiコンテナが再作成され、セッション（プロセスメモリ）がリセットされているため再ログインする。
e2e_api_login "$BASE"
for _ in $(seq 1 60); do api GET /v1/providers >/dev/null 2>&1 && break; sleep 1; done
