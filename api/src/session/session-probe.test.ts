...
const { loadVendorProfile } = await import("../profile/profile-loader.js");

const account = loadVendorProfile().actions.account!;
...
describe("getSessionInfo", () => {
  ...
  it("判定結果を30秒間キャッシュし、同じ期間の再取得ではCLIを起動しない", async () => {
    ...
    const first = await getSessionInfo();
    const second = await getSessionInfo();
  ...
  it("invalidateSessionInfoで破棄すると再判定する", async () => {
    ...
    expect((await getSessionInfo()).plan?.id).toBe("paid");
  ...
  it("同時に来た要求は1回のCLI実行にまとめる", async () => {
    ...
    const results = await Promise.all([getSessionInfo(), getSessionInfo(), getSessionInfo()]);
    expect(executeVendorCommandMock).toHaveBeenCalledTimes(1);
  ...
  it("プロキシ未応答などで実行に失敗したら不明（空）を返し、キャッシュしない", async () => {
    ...
    expect(await getSessionInfo()).toEqual({});
    executeVendorCommandMock.mockResolvedValueOnce({ exitCode: 0, stdout: PAID_OUTPUT, stderr: "" });
    expect((await getSessionInfo()).loggedIn).toBe(true);
  });
  ...
  it("標準エラーだけに未ログインの旨が出ても未ログインと判定する", async () => {
    ...
    expect(await getSessionInfo()).toEqual({ loggedIn: false });
  });
});
