...
let cached: { info: SessionInfo; expiresAt: number } | undefined;
// 実行中の判定。同時に来た要求を1回のCLI実行へまとめる。
...
/**
 * 目的: 現在のログイン状態・プランを取得する（30秒キャッシュ）。
 * 入力: なし。
...
/**
 * 目的: キャッシュ済みのログイン状態・プランを破棄する。
 * 副作用: ログイン・ログアウトの成功時に呼ぶ（状態が変わったため次の取得で再判定させる）。
 */
export function invalidateSessionInfo(): void {
  cached = undefined;
}
