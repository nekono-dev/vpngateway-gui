...
import { getLoginMethod, loadVendorProfile } from "../profile/profile-loader.js";
 ...
 */
function resetSessionDerivedState(): void {
...
export const registerSessionRoute: FastifyPluginAsyncTypebox = async (fastify) => {
  fastify.get(
    ...
    async () => {
      ...
      return {
        loginMethod: getLoginMethod(profile),
  ...
  fastify.post(
    ...
    async (request) => {
      ...
      if (getLoginMethod(profile) === "credentials") {
        ...
        appendAuditLog({ action: "login", input: { username: credentials.username }, exitCode });
        if (exitCode !== 0) {
  ...
  fastify.delete(
    ...
    async () => {
      ...
      const exitCode = result.exitCode ?? -1;
      appendAuditLog({ action: "logout", exitCode });
      if (exitCode !== 0) {
