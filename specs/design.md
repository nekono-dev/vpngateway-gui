...
- `web`・`api`・`gateway`（`proxy`＋有効化した`runner-<ベンダー>`）の3ロールへ、composeファイルを分割する。
  ...
  - ルートの`docker-compose.yml`は、全ロールを1ホストへまとめる既定形向けに3ファイルをまとめて含む薄いラッパーとする。
- 各ホストの`.env`の`COMPOSE_FILE`には、そのホストに配置されたロールのcomposeファイルだけを並べる（ベンダーの有効化と同じ、追加ファイルの合成方式）。どのロールをどのホストへ配置するかは、下記「オーケストレーション型インストーラ」がまとめて決定する。
...
- オーケストレーターは、作業用ディレクトリ（例 `$VPNGW_DIR/.pki-staging/`。配布後に削除する）へ上記の証明書一式を生成し、各ロールの配置先（ローカルまたは`scp`先）の所定のパス（`/etc/vpngwgui/pki/`）へ配置する。CA秘密鍵は配布物に含めない（配布するのはCA“証明書”と、各サーバ・クライアントの証明書・秘密鍵のみ）。
...
- **TLS**: TLS 1.3以上。Node.js組み込みの`tls`/`https`モジュールを使う（外部フレームワーク不要という既存方針を踏襲）。サーバは`requestCert: true`・`rejectUnauthorized: true`とし、APIサーバのクライアント証明書がゲートウェイのCA（上記「証明書の生成・配布」）で発行されたものでなければ接続を拒否する。
