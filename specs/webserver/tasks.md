...
- [x] 接続国ドロップダウン実装（プロファイル定義の選択肢のみ、自由入力不可。**Phase 8で接続先リストへ置換**）。ベンダー選択UIは実装しない（1サーバー1ベンダーの設計方針、apiserver/requirements.md参照）
- [x] 接続/切断ボタン実装（二重送信防止・ローディング表示）

## 画面: 接続操作の刷新 (Phase 8)

`wbs/phase8.md`。設計は`design.md`「接続先リストの実装方針（Phase 8）」。
...
- [x] コンポーネントテスト
- [x] 実VPNでのE2E（`e2e/phase8/`。2026-09-21実施、`wbs/phase8.md`「検証結果」参照）
...
- [x] ~~`defaultCountry` ドロップダウン~~（**Phase 8で廃止**。下記参照）実装（ダッシュボードの接続国選択と同じ`GET /v1/connection/countries`の一覧を再利用。1サーバー1ベンダー運用のため`GET /v1/vendors/{vendor}/countries`は実装しない）
