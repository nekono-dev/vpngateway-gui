# Phase 12: プランで接続できる接続先の参考表示

（2026-09-21追加: Phase 9（Proton VPN）の実機検証で、無料版は接続先を選べず自動接続のみだと分かった。利用者から「無料版で接続できる国の情報を取得して、Web UIに表示したい」という要望があり、Phase 11の後に追加する。Phase 10でベンダーバンドル化した`docker-compose.yml`の記述（キャッシュのマウント）とプロファイルの宣言は、そのバンドルへ移してある。）
...
- Phase 7（プラン制限の判定）・Phase 9（Proton VPNのランナー・ログイン）完了。
...
### 設計・仕様（実装前）
- [x] 要件・設計・タスクの記述（`specs/requirements.md`・`specs/apiserver/`・`specs/webserver/`）。**実装は、先に実装へ着手してしまい、指摘を受けて文書を先に書いた。設計は`Phase 10`（ベンダー非依存）の方針に合わせ、ベンダー固有の形式をコードに持たず、プロファイルの宣言で表す形にした。**
...
- [x] 実機のWeb UI（ブラウザ）での表示確認（2026-09-22。検証環境`192.168.3.240`でベンダーをProton VPN（無料アカウント・ログイン済み）へ切替え、Playwright（`e2e/phase12/webgui-available-locations.mjs`）で「接続できる国（参考）」の一覧表示・件数の一致・先頭の国と都市の表示・操作可能要素が無いこと・自動接続の説明文をすべて確認（7項目PASS）。確認後、ベンダーをAdGuard VPNへ戻した）。
...
- （実装中に判明した点）(1)検証環境で、利用者が別途Proton VPNへ再ログインしていた（一覧はログイン中のプランでのみ返る。未ログイン・有料は空）。(2)国名の並びは`Intl.DisplayNames`（ja）・五十音順（アメリカ合衆国が日本より前）。(3)Phase 10のバンドル化で、`docker-compose.yml`の`api`のマウント（`proton-cache`→`<PROVIDER_CACHE_DIR>/protonvpn`）はバンドルのcompose fragmentへ移す。
