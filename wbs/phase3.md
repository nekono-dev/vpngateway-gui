...
- Phase2完了（proxyコンテナが`network_mode: host`で稼働しており、LANインターフェースへ直接bindできる状態）。
- Phase2で追加した内部プロトコル`POST /settings`（設定反映エンドポイント）が利用可能であること。

## スコープ外

- 実VPNベンダーCLIへの置換はしない（phase4.md）。
